...
class RemoteFileTest < ActiveSupport::TestCase
  ...
  test "#read_content" do
    ...
    assert_equal "pants", @file.read_content
  end

  test "#read_content on a file that does not exists on s3" do
    stub_s3(@key, method: :get, status: 404)
    assert_nil @file.read_content
  end
  ...
  def stub_s3(key, method: :head, status: 200, body: "")
    stub_request(method, "#{Remets.aws_bucket.url}/#{key}")
